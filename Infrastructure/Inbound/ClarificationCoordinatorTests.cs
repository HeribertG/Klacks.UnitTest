...
public class ClarificationCoordinatorTests
{
    ...
    [Test]
    public async Task After_RecipientLongerThanTheColumn_IsTreatedAsNoPersonalContact_AndNothingIsWritten()
    {
        var tooLong = new string('a', InboundClarificationConstants.MaxRecipientLength + 1);
        _emailSender.ResolveTargetAsync(Arg.Any<ClarificationRequest>(), Arg.Any<CancellationToken>())
            .Returns(new InboundReplyTarget(Recipient: tooLong, Subject: "Re: Krank", InReplyTo: null, References: null));

        var result = await _coordinator.AfterAnalysisAsync(
            EmailRequest(new ClarificationEmailThread(MessageId: "<m-1@example.com>", InReplyTo: null, ThreadReferences: null, IsAutoGenerated: false)),
            UnclearAnalysis());
    ...
    [Test]
    public async Task After_RecipientExactlyAtTheColumnLimit_IsStillAsked()
    {
        var atLimit = new string('a', InboundClarificationConstants.MaxRecipientLength);
        _messengerSender.ResolveTargetAsync(Arg.Any<ClarificationRequest>(), Arg.Any<CancellationToken>())
            .Returns(new InboundReplyTarget(Recipient: atLimit, Subject: null, InReplyTo: null, References: null));

        await _coordinator.AfterAnalysisAsync(MessengerRequest(), UnclearAnalysis());

        await _repository.Received(1).TryAddOpenAsync(
            Arg.Is<InboundClarification>(c => c.Recipient == atLimit), Arg.Any<CancellationToken>());
    }
    ...
    private async Task<ClarificationPreAnalysis> LoseTheAnswerRaceAgainst(InboundClarificationStatus? statusOnReload, bool reloadedHasAnswerSource = true)
    {
        ...
        return await _coordinator.BeforeAnalysisAsync(MessengerRequest());
    }

    [Test]
    public async Task Before_SweepWonTheRace_ReturnsExpiryNote_AndTheMessageIsAnalyzedNormally()
    {
        var result = await LoseTheAnswerRaceAgainst(InboundClarificationStatus.Expired);
    ...
    [Test]
    public async Task Before_PlannerTookOverDuringTheAnswerAnalysis_NoteDoesNotClaimExpiry()
    {
        var result = await LoseTheAnswerRaceAgainst(InboundClarificationStatus.TakenOver);
        ...
        result.NotifierContext.ShouldContain("taken over by a planner");
    }

    [TestCase(InboundClarificationStatus.Answered)]
    [TestCase(InboundClarificationStatus.Unresolved)]
    public async Task Before_ParallelAnswerWonTheRace_NoteDoesNotClaimExpiry(InboundClarificationStatus reloadedStatus)
    {
        var result = await LoseTheAnswerRaceAgainst(reloadedStatus);
    ...
    public async Task Before_RaceLostButRowMissingOrStillOpen_ReturnsNoNote(InboundClarificationStatus? reloadedStatus)
    {
        var result = await LoseTheAnswerRaceAgainst(reloadedStatus);
    ...
    [Test]
    public async Task Before_AutoGeneratedMail_NeverClosesAnOpenRound()
    {
        _repository.GetOpenByClientAsync(ClientId, Arg.Any<CancellationToken>()).Returns(OpenClarification());
        var request = EmailRequest(new ClarificationEmailThread("ooo-1@example.com", "<original-1@example.com>", null, IsAutoGenerated: true));
        ...
        await _repository.DidNotReceiveWithAnyArgs().TryResolveAsync(
            default, default, answerSourceId: default, resultAnalysisId: default, resolvedAtUtc: default, cancellationToken: default);
    }
